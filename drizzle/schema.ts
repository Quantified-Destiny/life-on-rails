import { mysqlTable, mysqlSchema, AnyMySqlColumn, uniqueIndex, index, varchar, text, int, datetime, mysqlEnum, double, tinyint } from "drizzle-orm/mysql-core"
...
export const goal = mysqlTable("Goal", {
	...
	archivedAt: datetime("archivedAt", { mode: 'string', fsp: 3 }).default(sql`(CURRENT_TIMESTAMP(3))`).notNull(),
},
(table) => {
	return {
...
export const goalTag = mysqlTable("GoalTag", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
...
(table) => {
	return {
		tagIdGoalIdKey: uniqueIndex("GoalTag_tagId_goalId_key").on(table.tagId, table.goalId),
...
export const habit = mysqlTable("Habit", {
	...
	frequencyHorizon: mysqlEnum("frequencyHorizon", ['DAY','WEEK']).default('DAY').notNull(),
...
export const habitCompletion = mysqlTable("HabitCompletion", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
...
export const habitMeasuresGoal = mysqlTable("HabitMeasuresGoal", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	goalId: varchar("goalId", { length: 191 }).notNull(),
	habitId: varchar("habitId", { length: 191 }).notNull(),
...
(table) => {
	return {
		goalIdHabitIdKey: uniqueIndex("HabitMeasuresGoal_goalId_habitId_key").on(table.goalId, table.habitId),
...
export const habitTag = mysqlTable("HabitTag", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	habitId: varchar("habitId", { length: 191 }).notNull(),
	tagId: varchar("tagId", { length: 191 }).notNull(),
},
(table) => {
	return {
		tagIdHabitIdKey: uniqueIndex("HabitTag_tagId_habitId_key").on(table.tagId, table.habitId),
		habitIdIdx: index("HabitTag_habitId_idx").on(table.habitId),
		tagIdIdx: index("HabitTag_tagId_idx").on(table.tagId),
	}
});

export const linkedMetric = mysqlTable("LinkedMetric", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	date: datetime("date", { mode: 'string', fsp: 3 }).default(sql`(CURRENT_TIMESTAMP(3))`).notNull(),
	habitId: varchar("habitId", { length: 191 }).notNull(),
...
export const metric = mysqlTable("Metric", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
...
export const metricAnswer = mysqlTable("MetricAnswer", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
...
(table) => {
	return {
		...
		habitCompletionIdIdx: index("MetricAnswer_habitCompletionId_idx").on(table.habitCompletionId),
	}
});

export const metricMeasuresGoal = mysqlTable("MetricMeasuresGoal", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	goalId: varchar("goalId", { length: 191 }).notNull(),
	metricId: varchar("metricId", { length: 191 }).notNull(),
	weight: int("weight").default(1).notNull(),
...
(table) => {
	return {
		goalIdMetricIdKey: uniqueIndex("MetricMeasuresGoal_goalId_metricId_key").on(table.goalId, table.metricId),
...
export const metricTag = mysqlTable("MetricTag", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	metricTag: varchar("metricTag", { length: 191 }).notNull(),
	tagId: varchar("tagId", { length: 191 }).notNull(),
},
(table) => {
	return {
		tagIdMetricTagKey: uniqueIndex("MetricTag_tagId_metricTag_key").on(table.tagId, table.metricTag),
		metricTagIdx: index("MetricTag_metricTag_idx").on(table.metricTag),
...
(table) => {
	return {
		ownerIdNameKey: uniqueIndex("Tag_ownerId_name_key").on(table.ownerId, table.name),
...
export const user = mysqlTable("User", {
	...
	scoringUnit: mysqlEnum("scoringUnit", ['Percentage','Normalized']).default('Normalized').notNull(),
},
(table) => {
	return {
